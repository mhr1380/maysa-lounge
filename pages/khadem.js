    ...
    const fetchRestaurantDetails = async () => {
      try {
        const response = await axios.get(
          "https://fancymenu.ir/wp-json/wp/v2/restaurant/41?_fields=menu_items,menu_category,meta,title"
        );
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="منوی دیجیتال رستوران بزرگ خادم" />
        <link rel="icon" href="/khadem.png" sizes="any" />
      </Head>
      {dataLoaded ? (
        <div className="min-h-screen flex flex-col">
          ...
          <div className="bg-[#EEEDEB] dark:bg-[#2d2d2d] flex flex-col pt-2">
            ...
            {categories.filter((cat) => cat.menuItems.length > 0).length ===
            0 ? (
              <>
                <span className="flex justify-center px-2 items-center gap-x-1 mb-4">
                  <span className="w-[22%] h-[2px] bg-gray-400"></span>
...
const renderBeforeDataLoaded = () => {
  ...
  const greeting = `${getCurrentTime()} زیباتون بخیر.`;

  return (
    <div className="flex flex-col justify-center items-center h-screen w-full text-white -mt-16">
      <Image src={logo} alt="maysa" className="w-28 rounded-full" />
      <p className="mt-3">{greeting}</p>
      <p className="mt-1">{" به رستوران بزرگ خادم خوش اومدید :)"}</p>
